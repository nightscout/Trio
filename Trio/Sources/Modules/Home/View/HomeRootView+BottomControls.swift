...
extension Home.RootView {
    ...
    /// Bolus plus temp basal insulin delivered since midnight; banner-grade, no scheduled-basal inference.
    private var todayInsulinDelivered: Double {
        let startOfDay = Calendar.current.startOfDay(for: Date())
        let now = Date()

        let bolusTotal = state.insulinFromPersistence
            .filter { ($0.timestamp ?? .distantPast) >= startOfDay }
            .compactMap { $0.bolus?.amount?.doubleValue }
            .reduce(0, +)

        // schedule-change rows mark reprogramming, not delivery (see BasalChart)
        let deliveryEvents = state.tempBasals
            .filter { $0.tempBasal?.isScheduledBasal != true }
            .sorted { ($0.timestamp ?? .distantPast) < ($1.timestamp ?? .distantPast) }

        var basalTotal = 0.0
        for (index, event) in deliveryEvents.enumerated() {
            guard let tempBasal = event.tempBasal, let start = event.timestamp else { continue }

            let plannedEnd = tempBasal.endDate ?? start.addingTimeInterval(Int(tempBasal.duration).minutes.timeInterval)

            // a later-starting temp basal supersedes this one, same clipping as BasalChart
            var next = index + 1
            while next < deliveryEvents.count, (deliveryEvents[next].timestamp ?? .distantPast) <= start { next += 1 }
            let supersededEnd = next < deliveryEvents.count ? deliveryEvents[next].timestamp ?? plannedEnd : plannedEnd
            let effectiveEnd = min(plannedEnd, supersededEnd, now)

            let overlapStart = max(start, startOfDay)
            let overlapSeconds = effectiveEnd.timeIntervalSince(overlapStart)
            guard overlapSeconds > 0 else { continue }

            let runSeconds = effectiveEnd.timeIntervalSince(start)
            if let delivered = tempBasal.deliveredUnits?.doubleValue, runSeconds > 0 {
                // finalized rows carry actual delivery; prorate midnight-spanning runs
                basalTotal += delivered * overlapSeconds / runSeconds
            } else {
                basalTotal += (tempBasal.rate?.doubleValue ?? 0) * overlapSeconds / 3600
            }
        }
    ...
    /// Carbs, fat and protein announced since midnight (original entries only, converted FPUs excluded by the fetch).
    private var todayMeals: (carbs: Double, fat: Double, protein: Double) {
        let startOfDay = Calendar.current.startOfDay(for: Date())
        return state.carbsFromPersistence
            .filter { ($0.date ?? .distantPast) >= startOfDay }
            .reduce((carbs: 0, fat: 0, protein: 0)) {
                (carbs: $0.carbs + $1.carbs, fat: $0.fat + $1.fat, protein: $0.protein + $1.protein)
            }
    }

    @ViewBuilder private func treatmentStat(icon: String, tint: Color, value: String) -> some View {
        HStack(spacing: 4) {
    ...
    private func treatmentsAccessibilityValue(
        insulin: String,
        carbs: String?,
        fat: String?,
        protein: String?
    ) -> String {
        var parts = [String(localized: "\(insulin) insulin", comment: "Accessibility: treatments face")]
        if let carbs { parts.append(String(localized: "\(carbs) carbs", comment: "Accessibility: treatments face")) }
        if let fat { parts.append(String(localized: "\(fat) fat", comment: "Accessibility: treatments face")) }
        if let protein { parts.append(String(localized: "\(protein) protein", comment: "Accessibility: treatments face")) }
        return parts.joined(separator: ", ")
    }

    // fat/protein have no established icon; a tinted word label stands in (FPU brown, like the chart dots)
    @ViewBuilder private func treatmentStat(label: Text, tint: Color, value: String) -> some View {
        HStack(spacing: 4) {
            label
                .foregroundStyle(tint)
            Text(value)
            ...
            ZStack {
                HStack(alignment: .center, spacing: 12) {
                    ...
                    case .treatments:
                        ...
                        let gram = String(localized: " g", comment: "gram of carbs")
                        let insulinString = todayInsulinDelivered
                            .formatted(.number.precision(.fractionLength(1))) +
                            String(localized: " U", comment: "Insulin unit")
                        let carbsString = meals.carbs.formatted(.number.precision(.fractionLength(0))) + gram
                        let fatString = meals.fat.formatted(.number.precision(.fractionLength(0))) + gram
                        let proteinString = meals.protein.formatted(.number.precision(.fractionLength(0))) + gram

                        VStack(alignment: .leading, spacing: 6) {
                            HStack(spacing: 10) {
                                treatmentStat(icon: "syringe.fill", tint: .insulin, value: insulinString)
                                // meal stats only for people who announce them (#1474)
                                if meals.carbs > 0 {
                                    treatmentStat(icon: "fork.knife", tint: .loopYellow, value: carbsString)
                                }
                                if meals.fat > 0 {
                                    treatmentStat(
                                        label: Text("F", comment: "First letter of 'Fat' for stats banner label"),
                                        tint: .brown,
                                        value: fatString
                                    )
                                }
                                if meals.protein > 0 {
                                    treatmentStat(
                                        label: Text("P", comment: "First letter of 'Protein' for stats banner label"),
                                        tint: .brown,
                                        value: proteinString
                                    )
                                }
                            ...
                            Text("Today's Treatments", comment: "Stats banner subtitle")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                        .accessibilityElement(children: .ignore)
                        .accessibilityLabel(Text("Today's Treatments", comment: "Stats banner subtitle"))
                        .accessibilityValue(Text(treatmentsAccessibilityValue(
                            insulin: insulinString,
                            carbs: meals.carbs > 0 ? carbsString : nil,
                            fat: meals.fat > 0 ? fatString : nil,
                            protein: meals.protein > 0 ? proteinString : nil
                        )))
                    case .hidden:
